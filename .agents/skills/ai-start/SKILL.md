...
Редактор не спрашиваю. В чате чужие редакторы не называю. Скилл — туда, куда этот агент его читает. MCP — только в открытой папке, не в домашнем конфиге. Правило: [references/connect-mcp.md](references/connect-mcp.md).
...
2. Пишу сервер в **проектный** конфиг **этого** редактора (файл в открытой папке). Домашние `~/...` конфиги не трогаю. Чужие серверы не стираю. Не пишу сразу несколько редакторов «на всякий случай».
3. В чате одна реплика: я уже добавил **имя** в этот проект. Открой MCP, сервер уже в списке, нажми вход в браузере. Когда живой — **готово**. Если входа не было — **не вошло**.
...
5. Если этот редактор умеет сменить корень чата на папку — переключаю на `clone_path`. Сразу дописываю GitHub MCP уже в **новый** корень (правило connect-mcp). В том же ходе — шаг 5.
6. Если нет, в чате: я сделал тебе копию и положил её сюда: `<clone_path>`. Открой эту папку и напиши **готово**. Когда папка открыта — дописываю GitHub MCP в её корень, затем шаг 5.
